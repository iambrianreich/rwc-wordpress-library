...
namespace RWC\Metabox {
    ...
    abstract class Field extends \RWC\Object
    {
        ...
        public static function get_instance(\RWC\Metabox $metabox, $id, $options ) {
            ...
            $namespaces = $metabox->get_library()
                ->get_autoloader()->get_namespaces();

            $field = null;

            /*
             * Iterate through all of the namespaces configured for the library.
             * We'll try to load the Field type from all configured namespaces
             * so we catch site-specific types.
             */
            foreach( $namespaces as $namespace => $path )
            {
                // The full namespaces class name.
                $class = '\\' . $namespace . '\Metabox\Field\\' .
                    \RWC\Strings::get_camel_case( $options[ 'type' ] );

                // Squash exceptions. If it succeeds $field will have a value,
                // If it fails at each iteration it will be null.
                try
                {
                    // Try to load class first.
                    if( class_exists( $class )) {
                        $field = new $class( $options, $metabox );
                    }


                    // If we have a winner, return!
            ...
            /*
             * Field was not generated by iterating through configured
             * namespaces. Throw a Metabox\Exception explaining that the type
             * does not exist.
             */
            throw new Exception( 'Failed to load Metabox field type ' .
                $options[ 'type' ] . '. The type could not be found ' .
                'in any of the configured namespaces.' );
        }
    }
